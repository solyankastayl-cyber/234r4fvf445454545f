  ...
  useEffect(() => {
    ...
    if (patternViewMode && patternWindow && patternWindow.start && patternWindow.end) {
      ...
      if (mapped.length > 0 && isFirstLoadRef.current) {
        // Calculate bar duration to determine appropriate zoom level
        const barDuration = mapped.length > 1 ? mapped[1].time - mapped[0].time : 86400;
        
        // Determine visible bars based on timeframe:
        // 4H-6H (~14400-21600s) -> 60 bars = ~10-15 days - ALWAYS zoom for better view
        // 1D (86400s) -> 90 bars = ~3 months
        // 7D (604800s) -> 30 bars = ~7 months
        // 1M -> 24 bars = ~2 years
        let visibleBars;
        let shouldZoom = isRangePattern;  // Default: zoom only for range
        
        if (barDuration <= 21600) {
          // 4H-6H or smaller - ALWAYS show zoomed in (~10-15 days) for better UX
          visibleBars = 60;
          shouldZoom = true;  // Always zoom for 4H/6H
        } else if (barDuration <= 86400) {
          // 1D - show ~3 months if range, otherwise fitContent
          visibleBars = 90;
        } else if (barDuration <= 604800) {
          // 7D - show ~7 months
          visibleBars = 30;
        } else {
          // Monthly or larger - show ~2 years
          visibleBars = 24;
        }
        ...
        if (shouldZoom) {
          ...
          // Add padding for forward extension
          const paddedTo = toTime + barDuration * 10; // 10 bars padding
          
          console.log('[ResearchChart] Zooming to last', visibleBars, 'candles, barDuration:', barDuration, 'isRange:', isRangePattern);
          chart.timeScale().setVisibleRange({
            from: fromTime,
            to: paddedTo,
          });
        } else {
          // For larger timeframes without range - fitContent
